...
$(document).ready(function () {
        ...
        if ($('.Inputfield_enabledSubmodules').length) {
            ...
            var docLink = '<a class="docLink" target="_blank" title="Documentation"><i class="fa fa-question-circle"></a>',
                gitHubLink = 'https://github.com/rolandtoth/AdminOnSteroids#';

            $('input[id^="Inputfield_enabledSubmodules_"]').each(function () {
                var input = $(this),
                    submodule = input.parent().parent('li'),
                    submoduleName = input.attr('id').replace('Inputfield_enabledSubmodules_', '');

                docLink = $(docLink).clone().attr('href', gitHubLink + submoduleName.toLowerCase());

                submodule.prepend($(docLink));
            });
        }
